...
class Place {
  ...
  static async getAllForProvider(providerId) {
    const [rows] = await db.query(
      "SELECT * FROM archiver WHERE provider_id = ? ORDER BY createdAt DESC",
      [providerId]
    );
    return rows;
  }
  ...
  static async getByRegion(region) {
    const [rows] = await db.query(
      "SELECT * FROM archiver WHERE JSON_EXTRACT(location, '$.region') = ? ORDER BY createdAt DESC", 
      [region]
    );
    return rows;
  }
  ...
  static async getPopular(limit = 10) {
    const [rows] = await db.query(
      "SELECT * FROM archiver ORDER BY average_rating DESC LIMIT ?", 
      [limit]
    );
    return rows;
  }
  ...
  static async create(placeData) {
    ...
    const [result] = await db.query(
      ...
      (name, type, description, location, provider_id)
      VALUES (?, ?, ?, ?, ?)`,
      [
        placeData.name,
        placeData.type,
  ...
  static async update(id, updates) {
    ...
    const [result] = await db.query(`UPDATE archiver SET ${fields} WHERE id = ?`, values);
    return result.affectedRows > 0;
  }

  // Supprimer un lieu
  static async delete(id) {
    const [result] = await db.query("DELETE FROM archiver WHERE id = ?", [id]);
    return result.affectedRows > 0;
  }
  ...
  static async getAllByProvider(providerId) {
    const [rows] = await db.query(
      "SELECT * FROM archiver WHERE provider_id = ?", 
      [providerId]
    );
    return rows;
  }
